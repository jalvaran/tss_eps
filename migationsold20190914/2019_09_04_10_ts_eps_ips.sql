...
SELECT t2.ID,t2.NumeroFactura,t2.Estado,t2.DepartamentoRadicacion,t1.NoRelacionada,
        ...
        (SELECT IFNULL((SELECT SUM(ValorAnticipado) FROM anticipos2 WHERE anticipos2.NumeroFactura=t2.NumeroFactura AND (NumeroInterno='2039' OR NumeroInterno='2259')),0)) AS FacturasDevueltas,
        ...
        (SELECT IFNULL((SELECT COUNT(DISTINCT NumeroTransaccion) FROM notas_db_cr_2 WHERE notas_db_cr_2.NumeroFactura=t2.NumeroFactura AND notas_db_cr_2.C13<>'N' AND ( notas_db_cr_2.TipoOperacion='2269' OR notas_db_cr_2.TipoOperacion='2039') ),0)) AS DevolucionesPresentadas,
        ...
        (SELECT IF(FacturaActiva='SI',0,(SELECT IFNULL((SELECT (ValorTotal) FROM notas_db_cr_2 WHERE notas_db_cr_2.NumeroFactura=t2.NumeroFactura AND (notas_db_cr_2.TipoOperacion='2259' OR notas_db_cr_2.TipoOperacion='2269' OR notas_db_cr_2.TipoOperacion='2039'  OR notas_db_cr_2.TipoOperacion='2088' ) AND notas_db_cr_2.FechaTransaccion>=t2.FechaRadicado LIMIT 1),0))) ) AS TotalDevolucionesNotas,
...
SELECT (SELECT notas_db_cr_2.NumeroTransaccion) AS Numero_concidencia,(SELECT anticipos2.NumeroFactura) AS Numerofactura_anticipos,(SELECT notas_db_cr_2.NumeroFactura) AS Numerofactura_cxp,(SELECT notas_db_cr_2.TipoOperacion),(SELECT anticipos2.NumeroInterno)
FROM anticipos2
LEFT JOIN
notas_db_cr_2 ON notas_db_cr_2.NumeroTransaccion=anticipos2.NumeroAnticipo AND notas_db_cr_2.TipoOperacion=anticipos2.NumeroInterno WHERE notas_db_cr_2.TipoOperacion='2259';