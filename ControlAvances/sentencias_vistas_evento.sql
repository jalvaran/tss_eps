...
SELECT t2.ID,t2.NumeroFactura,t2.Estado,t2.DepartamentoRadicacion,t1.NoRelacionada,
		...
		(SELECT IFNULL((SELECT SUM(ValorAnticipado) FROM anticipos2 WHERE anticipos2.NumeroFactura=t2.NumeroFactura AND (NumeroInterno='2216' or NumeroInterno='2117' or NumeroInterno='2254' or NumeroInterno='2217') ),0)) AS TotalAnticipos,
...
DROP VIEW IF EXISTS `vista_cruce_cartera_epsnitips`;
CREATE VIEW vista_cruce_cartera_epsnitips AS
SELECT t2.Nit_IPS,t2.ID,t2.NumeroFactura,t2.Estado,t2.DepartamentoRadicacion,(SELECT NoRelacionada FROM carteracargadaips WHERE carteracargadaips.NumeroFactura=t2.NumeroFactura LIMIT 1) as NoRelacionada,
...
DROP VIEW IF EXISTS `resumen_vista_cruce_cartera_eps`;
CREATE VIEW resumen_vista_cruce_cartera_eps AS
select distinct NumeroContrato, COUNT(NumeroFactura) AS CantidadFacturas, Nit_IPS AS NitIps FROM vista_cruce_cartera_epsnitips GROUP BY NumeroContrato