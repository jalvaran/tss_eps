...
class RevisionFacturas extends conexion{
    ...
    public function ConstruirTablaDeFacturasConCerosIzquierda($db,$idUser) {
        ...
        $sql="CREATE TABLE $db.facturas_para_revision_por_ceros_izquierda AS
            ...
            FROM $db.historial_carteracargada_eps t1 WHERE EXISTS (SELECT 1 FROM $db.vista_espejo_tibco t2 WHERE t1.ValidaFactura=t2.ValidaFactura) ORDER BY ValidaFactura";
        $this->Query($sql);
    }
    ...
    public function ActualizarCantidadFacturasConCeros($db,$idUser) {
        ...
        $sql="update $db.historial_carteracargada_eps t1 
            ...
            WHERE t1.ValidaFactura >0  and t1.Numerofactura like '0%';
            ";
        $this->Query($sql);
    }
