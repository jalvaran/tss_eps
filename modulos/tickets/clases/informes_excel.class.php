...
class ExcelReportes extends conexion{
    ...
    public function informe_gestion($FechaInicial,$FechaFinal,$CmbEstado,$CmbProyectosTicketsListado,$CmbModulosTicketsListado,$CmbTiposTicketsListado,$usuario_id) {
     ...
     //Resumen de Tickets
        ...
        $z=0;
        $objPHPExcel->setActiveSheetIndexByName($nombre_hoja3)
            ->setCellValue($Campos[$z++].$i,"PROYECTO")    
            ->setCellValue($Campos[$z++].$i,"MODULO")
            ->setCellValue($Campos[$z++].$i,"TIPO DE TICKET")
            ->setCellValue($Campos[$z++].$i,"ESTADO")
            ->setCellValue($Campos[$z++].$i,"TOTAL")
            ->setCellValue($Campos[$z++].$i,"PROMEDIO RESPUESTAS (Hrs.)")
            ->setCellValue($Campos[$z++].$i,"PROMEDIO CIERRE (Hrs.)")
            
            ;
        
        $sql="SELECT COUNT(ID) as Total,AVG(horas_respuesta) as promedio_respuesta,AVG(horas_cierre) as promedio_cierre, NombreProyecto,NombreModulo,NombreTipoTicket,NombreEstado
        ...
        while ($datos_consulta = $this->FetchAssoc($Consulta)) {
            $i++;
            $z=0;
            $total_tickets=$total_tickets+$datos_consulta["Total"];
            $objPHPExcel->setActiveSheetIndexByName($nombre_hoja3)
                ->setCellValue($Campos[$z++].$i,utf8_encode($datos_consulta["NombreProyecto"]))    
                ...
                ->setCellValue($Campos[$z++].$i,utf8_encode($datos_consulta["Total"]))
                ->setCellValue($Campos[$z++].$i,utf8_encode(round($datos_consulta["promedio_respuesta"],2)))
                ->setCellValue($Campos[$z++].$i,utf8_encode(round($datos_consulta["promedio_cierre"],2)))
        ...
        $objPHPExcel->getActiveSheet()->getStyle('A3:G3')->applyFromArray($styleTitle);
        ...
        $objPHPExcel->setActiveSheetIndexByName($nombre_hoja4)
            ...
            ->setCellValue($Campos[$z++].$i,"USUARIO ASIGNADO")   
            ...
            ->setCellValue($Campos[$z++].$i,"CIERRE (Hrs)")
        ...
        while ($datos_consulta = $this->FetchAssoc($Consulta)) {
            ...
            $objPHPExcel->setActiveSheetIndexByName($nombre_hoja4)
                ...
                ->setCellValue($Campos[$z++].$i,utf8_encode($datos_consulta["NombreAsignado"]." ".$datos_consulta["ApellidoAsignado"]))       
                ->setCellValue($Campos[$z++].$i,utf8_encode($datos_consulta["Asunto"]))
                ->setCellValue($Campos[$z++].$i,utf8_encode($datos_consulta["NombreEstado"]))
                ->setCellValue($Campos[$z++].$i,utf8_encode($datos_consulta["NombreProyecto"]))    
                ->setCellValue($Campos[$z++].$i,utf8_encode($datos_consulta["NombreModulo"]))
                ->setCellValue($Campos[$z++].$i,utf8_encode($datos_consulta["NombreTipoTicket"]))
                ->setCellValue($Campos[$z++].$i,utf8_encode($datos_consulta["horas_respuesta"]))
                ->setCellValue($Campos[$z++].$i,utf8_encode($datos_consulta["horas_cierre"]))
                   
                
                ;
            
            $array_mensaje= explode("|", $mensaje);
                $m=0;
                $r=0;
                foreach ($array_mensaje as $key => $value) {
                    $i++;
                    $z=1;
                    
                    if($r==0){
                        $num_respuesta="Solicitud: ";
                    }else{
                        $num_respuesta="Respuesta $r: ";
                    }
                    $r++;
                    $objPHPExcel->setActiveSheetIndexByName($nombre_hoja4)
                        ->setCellValue($Campos[$z++].$i,utf8_encode($num_respuesta))        
                        ->setCellValue($Campos[$z++].$i,utf8_encode($value))    
                    ;
        ...
        $objPHPExcel->getActiveSheet()->getColumnDimensionByColumn($c++)->setWidth('24');
        $objPHPExcel->getActiveSheet()->getColumnDimensionByColumn($c++)->setWidth('55');
        $objPHPExcel->getActiveSheet()->getColumnDimensionByColumn($c++)->setWidth('15');
        $objPHPExcel->getActiveSheet()->getColumnDimensionByColumn($c++)->setWidth('15');
        $objPHPExcel->getActiveSheet()->getColumnDimensionByColumn($c++)->setWidth('15');
        $objPHPExcel->getActiveSheet()->getColumnDimensionByColumn($c++)->setWidth('15');
