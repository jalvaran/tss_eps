    ...
    public function GuardeNotasEnTemporal($keyArchivo,$idIPS,$idEPS,$idUser) {
        ...
        for ($h=0;$h<$hojas;$h++){
            ...
            for ($i=1;$i<=$filas;$i++){
                ...
                foreach ($ColumnasTabla["Field"] as $key => $value) {
                    if($value=='ID'){
                        $_DATOS_EXCEL[$i][$value]="";
                        continue;
                    }
                    $c=$c+1;
                    $Dato=$objPHPExcel->getActiveSheet()->getCell($Cols[$c].$i)->getCalculatedValue();
                    if($value=="FechaTransaccion" or $value=="FechaTasa" or $value=="FechaValidacionImpuesto" or $value=="C55" or $value=="C56" or $value=="C73"){
                        $cell=$cell = $objPHPExcel->getActiveSheet()->getCell($Cols[$c].$i);
                        if(PHPExcel_Shared_Date::isDateTime($cell)){
                            $Dato=PHPExcel_Shared_Date::ExcelToPHP($objPHPExcel->getActiveSheet()->getCell('A'.$i)->getValue());
                        }
        ...
        foreach($_DATOS_EXCEL as $campo => $valor){
            ...
            foreach ($valor as $campo2 => $valor2){
                $campo2 == "Sync" ? $sql.= $valor2."')," : $sql.= $valor2."','";
            }
