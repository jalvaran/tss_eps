...
class CargarDevolucionesPendientes extends conexion{
    
    public function getKeyFile($FechaCorteCartera,$CmbIPS,$CmbEPS) {
        $Fecha= str_replace("-", "", $FechaCorteCartera);
        return("devoluciones_pendientes_".$CmbEPS."_".$CmbIPS."_".$Fecha);
    }
    ...
    public function LeerArchivo($keyArchivo,$FechaCorte,$idIPS,$Separador,$idUser) {
        ...
        $CamposDatos["Valor"]=$z++;        
        $CamposDatos["NumeroRegistros"]=$z++;                       
        $CamposDatos["Aplicados"]=$z++;                      //10
        $CamposDatos["Errores"]=$z++;
        $CamposDatos["NoEnviados"]=$z++;
        $CamposDatos["Soporte"]=$z++;
        $CamposDatos["idUser"]=$z++;                                //15
        $CamposDatos["FechaRegistro"]=$z++;
        $CamposDatos["keyFile"]=$z++;
        $sqlCampos = "INSERT INTO $db.`temp_devoluciones_pendientes` (";
        $sqlValores= ' VALUES ';   
