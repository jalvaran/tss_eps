...
include_once("../clases/carteraxedades.class.php");

if( !empty($_REQUEST["Accion"]) ){
    $obCon = new CarteraXEdades($idUser);
    ...
    switch ($_REQUEST["Accion"]) {
        ...
        case 2: //Recibir el archivo
            ...
            $DatosCargas=$obCon->DevuelveValores("ips", "NIT", $CmbIPS);
            $db=$DatosCargas["DataBase"];
            $obCon->VaciarTabla("$db.temporal_carteraxedades");
            $destino='';
        ...
        case 5://Insertar registros nuevos
            ...
            $DatosCargas=$obCon->DevuelveValores("ips", "NIT", $CmbIPS);
            $db=$DatosCargas["DataBase"];
            $obCon->VaciarTabla("$db.carteraxedades");
            $sql="INSERT INTO $db.`carteraxedades`  
                   SELECT *
                  FROM $db.`temporal_carteraxedades` as t1;
