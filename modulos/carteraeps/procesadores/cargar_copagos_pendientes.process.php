...
include_once("../clases/cargar_copagos_pendientes.class.php");

if( !empty($_REQUEST["Accion"]) ){
    $obCon = new CargarCopagosPendientes($idUser);
    ...
    switch ($_REQUEST["Accion"]) {
        ...
        case 2: //Recibir el archivo
            ...
            $obCon->VaciarTabla("$db.temp_copagos_pendientes");
            $destino='';
        ...
        case 4://LLevar elos registros nuevos en la temporal al historial de cargas
            ...
            $DatosCargas=$obCon->DevuelveValores("ips", "NIT", $CmbIPS);
            $db=$DatosCargas["DataBase"];
            $obCon->VaciarTabla("$db.copagos_pendientes");
            $sql="INSERT INTO $db.`copagos_pendientes` 
                    SELECT * FROM $db.`temp_copagos_pendientes`";
            $obCon->Query($sql);
