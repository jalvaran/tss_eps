...
include_once("../clases/cargar_devoluciones_pendientes.class.php");

if( !empty($_REQUEST["Accion"]) ){
    $obCon = new CargarDevolucionesPendientes($idUser);
    ...
    switch ($_REQUEST["Accion"]) {
        ...
        case 2: //Recibir el archivo
            ...
            $obCon->VaciarTabla("$db.temp_devoluciones_pendientes");
            $destino='';
        ...
        case 4://LLevar elos registros nuevos en la temporal al historial de cargas
            ...
            $DatosCargas=$obCon->DevuelveValores("ips", "NIT", $CmbIPS);
            $db=$DatosCargas["DataBase"];
            $obCon->VaciarTabla("$db.devoluciones_pendientes");
            $sql="INSERT INTO $db.`devoluciones_pendientes` 
                    SELECT * FROM $db.`temp_devoluciones_pendientes`";
            $obCon->Query($sql);
