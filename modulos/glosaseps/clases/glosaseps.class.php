...
class GlosasEPS extends conexion{
    
    public function getKeyArchivoEPS($FechaCorteCartera,$CmbIPS,$CmbEPS) {
        $Fecha= str_replace("-", "", $FechaCorteCartera);
        return("glosas_eps_".$CmbEPS."_".$CmbIPS."_".$Fecha);
    }
            ...
            while (($data = fgetcsv($handle, 1000, $Separador)) !== FALSE) {
               ...
               if($i==2){
                   ...
                   if(count($data)<>$NumeroColumnasEncabezado){
                       exit("E1;<h3>El archivo enviado no corresponde al formato de glosas de EPS ASMET</h3>");
                   }
               }
        ...
        while (($data = fgetcsv($handle, 1000, $Separador)) !== FALSE) {
            ...
            if($data[5]<>""){
                ...
                if(count($FechaArchivo)>1){
                    $FechaFormato= $FechaArchivo[2]."-".$FechaArchivo[1]."-".$FechaArchivo[0];
                }else{
                    $FechaFormato=$data[5];
                }
             ...
             if($i>=2){
                 ...
                 $sql="INSERT INTO $db.`temporal_glosaseps_asmet` (  `Sede`,`Nit_IPS`,`RazonSocial`, `NumeroRadicado`, `NumeroFactura`,`FechaRadicado`,  `ValorFactura`, `ValorTotalGlosa`, `ValorGlosaFavor`, `ValorGlosaContra`, `ValorPendienteResolver`,  `Soporte`,`idUser`, `FechaRegistro`, `FechaActualizacion`) VALUES ";
                 $i=0;
