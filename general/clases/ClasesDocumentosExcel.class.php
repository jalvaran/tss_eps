   ...
   $objPHPExcel->
    ...
    public function GenerarAnexoActaLiquidacionXFacturas1($db,$DatosIPS,$idActaLiquidacion,$DatosActa,$TipoConsulta) {
        ...
        */
        ...
        $i++;
        $i++;
        if($DatosActaTipo["tipo_anexo_pdf"]==3){   //Si es un acta tipo pgp excluimos las columnas devoluciones y recuperacion de impuestos
            $objPHPExcel->setActiveSheetIndex(0)
        ...
        if($TipoActa<>3){
            while($DatosVista= $this->FetchAssoc($Consulta)){
                ...
                if($DatosActaTipo["tipo_anexo_pdf"]==3){//Si es PGP
                    $objPHPExcel->setActiveSheetIndex(0)
    ...
    public function GenerarAnexoActaLiquidacionXFacturasCapita($db,$DatosIPS,$idActaLiquidacion,$DatosActa,$TipoConsulta) {
        ...
        */
        ...
        $objPHPExcel->setActiveSheetIndex(0)
            ...
            ->setCellValue($Campos[$z++].$i,$Totales["TotalPagos"]+$Totales["TotalOtrosDescuentos"])
            ->setCellValue($Campos[$z++].$i,$Totales["Saldo"])
        ...
        $objPHPExcel->setActiveSheetIndex(0)
            ...
            ->setCellValue($Campos[13].$i++,$TotalValorAPagarActaEjecucion)
            ...
            ->setCellValue($Campos[13].$i++,$SaldoFinal)
   ...
   $objPHPExcel->
    ...
    public function ExportarHojaTrabajoExcel($db,$tipo_negociacion,$CmbIPS) {
        ...
        foreach ($ColumnasSeleccionadas["Field"] as $key => $value) {
            if($tipo_negociacion<>'CAPITA' AND ($value=='NumeroAfiliadosLMA' OR $value=='NumeroDiasLMA' OR $value=='ValorPercapita' OR $value=='PorcentajePoblacional' OR $value=='ValorAPagarLMA' OR $value=='DescuentoReconocimientoBDUA' ) ){
                continue;
            }
            $sql.="$value,";
            $objPHPExcel->setActiveSheetIndex(0)
