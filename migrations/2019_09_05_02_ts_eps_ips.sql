...
SELECT t2.ID,t2.NumeroFactura,t2.Estado,t2.DepartamentoRadicacion,t1.NoRelacionada,
       ...
       /* (SELECT IFNULL((SELECT SUM(ValorDevuelto) FROM vista_facturasdvueltas_anticiposvscxp WHERE vista_facturasdvueltas_anticiposvscxp.Numerofactura_anticipos=t1.NumeroFactura AND TipoOperacionanticipos='2259' AND Numerofactura_anticipos!=Numerofactura_cxp),0)) AS FacDevueltasCXPVSANT,*/
		...
		(SELECT IFNULL((SELECT COUNT(Numerofactura_anticipos) FROM vista_facturasdvueltas_anticiposvscxp WHERE vista_facturasdvueltas_anticiposvscxp.Numerofactura_anticipos='DV*HI914222' AND TipoOperacionanticipos='2259' AND vista_facturasdvueltas_anticiposvscxp.Numerofactura_anticipos!=vista_facturasdvueltas_anticiposvscxp.Numerofactura_cxp),0)) AS FacturasDevueltasCXPVSANT,
